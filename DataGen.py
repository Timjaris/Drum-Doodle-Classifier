...
def init():
    ...
    for filename in os.listdir(songDir):
        ...
        #print("fs = ", fs)
        #signal = signal[:,0] #no longer necessary after downsampling
...
def dataGen(N, seconds, set): #timestep? 2d or 1d conv    
    ...
    for i in range(N):
        ...
        if np.random.randint(2):
            ...
            problem = Song[start:start+int(seconds*5512)] + Doodle[start:start+int(seconds*5512)]*2
            data.append(problem)
    ...
    X = np.array(data).reshape((N, int(seconds*5512), 1))
    Y = targets.reshape(N, 2)
    return X, Y 

"""
def dataGen2(N, seconds, set): #timestep? 2d or 1d conv    
    data = []  
    targets = []
    for i in range(N):
        #print("len = ", len(songs))
        ...
        if np.random.randint(2):
            ...
            targets.append(target)
    targets = np.array(targets)
    X = np.array(data).reshape((N, int(seconds*44100), 1))
    Y = targets.reshape(N, 2)
    return X, Y
"""
...
def getSongSnippets(N, secs):
    snips = []
    for i in range(N):
        n = np.random.randint(len(songs))
        #Correct song
        Song = songs[n].signal
        end = len(Song) - secs * 5512
        start = np.random.randint(end)
        snip = Song[start:start+int(secs*5512)]
        snips.append(snip)
